...
def compute_attrition_probability(df):
    ...
    ratio = df["salary_lakhs"] / level_medians.replace(0, np.nan)
...
def generate_demo_data(n=500, random_seed=42):
    ...
    df = pd.DataFrame({
        ...
        "level": levels, "department": depts, "tenure_months": tenure,
        "performance_rating": perf, "salary_lakhs": np.round(salary, 2),
        "age": age, "gender": gender, "promotion_readiness": readiness
    })
...
with st.sidebar:
    ...
    if hire_total_pct != 100:
        st.warning(f"Hire distribution = {hire_total_pct}%. Normalizing...")
        if hire_total_pct > 0:
...
# ---- LOAD DATA (with clean switching) ----
if st.session_state.data_source == "upload" and uploaded_file is not None:
    try:
        df = pd.read_csv(uploaded_file)
        df = compute_attrition_probability(df)
        df["manager_id"] = df.get("manager_id", -1)
        st.sidebar.success(f"✅ Loaded {len(df)} employees from upload")
    except Exception as e:
        st.error(f"❌ Error loading CSV: {e}")
        st.session_state.data_source = "demo"
        df = generate_demo_data()
        st.info("Fell back to demo data due to upload error")
elif st.session_state.data_source == "demo":
    df = generate_demo_data()
    st.sidebar.success("✅ Loaded 500 demo employees")
else:
    st.error("👆 Please select a data source in the sidebar (upload a file or check 'Use Demo Data')")
    st.stop()

# ---- BASELINE SUMMARY (Styled Cards) ----
st.subheader("📋 Baseline Workforce Summary")

with st.container(border=True):
    r1c1, r1c2, r1c3, r1c4 = st.columns(4)
    r1c1.metric("👥 Total Employees", f"{len(df)}")
    r1c2.metric("💰 Monthly Payroll", f"₹{df['salary_lakhs'].sum()/100:.2f} Cr")
    r1c3.metric("⚠️ High/Critical Risk", f"{(df['attrition_risk_bucket'].isin(['High','Critical'])).sum()}", 
                delta=f"{(df['attrition_risk_bucket'].isin(['High','Critical'])).mean()*100:.0f}%")
    r1c4.metric("🚀 Ready Now", f"{(df['promotion_readiness']=='Ready Now').sum()}")

    r2c1, r2c2, r2c3, r2c4 = st.columns(4)
    r2c1.metric("🏢 Largest Dept", f"{df['department'].value_counts().index[0]}")
    r2c2.metric("📅 Median Tenure", f"{df['tenure_months'].median():.0f} months")
    r2c3.metric("♀️ Female %", f"{(df['gender']=='F').mean()*100:.0f}%")
    r2c4.metric("🎂 Avg Age", f"{df['age'].mean():.0f} years")

# ---- ACTIVE ASSUMPTIONS BAR ----
with st.expander("📌 Active Assumptions & Model Methodology", expanded=False):
    a1, a2, a3, a4, a5 = st.columns(5)
    a1.markdown(f"**Hires/Month:** {planned_hires}")
    a2.markdown(f"**Promo Rate:** {promotion_rate:.0%}")
    a3.markdown(f"**Inflation:** {salary_inflation:.1f}%")
    a4.markdown(f"**Attrition Mult:** {attrition_multiplier:.2f}x")
    a5.markdown(f"**Backfill Delay:** {backfill_delay}mo")
...
with tc1:
    ...
    for _, row in td_hc.iterrows():
        fig_t1.add_trace(go.Bar(y=[row["Variable"]], x=[row["High"]], orientation="h", marker_color="#1f77b4", name=row["Variable"], showlegend=False))
...
with co2:
    ...
    fig_attr.add_trace(go.Scatter(x=hist["month"], y=hist["attrition"].cumsum(), name="Cumulative", mode="lines+markers", yaxis="y2", line=dict(color="#ff7f0e", width=2)))
    fig_attr.update_layout(title="Attrition Curve", xaxis_title="Month", yaxis_title="Monthly Exits", 
                           yaxis2=dict(title="Cumulative", overlaying="y", side="right"), height=350, template="plotly_white")
